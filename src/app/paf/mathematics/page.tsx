...
import { chapters } from "@/lib/data/paf/mathematics";

const createSlug = (title: string) => {
  return title.toLowerCase().replace(/ /g, '-').replace(/:/g, '').replace(/\(/g, '').replace(/\)/g, '');
};

export default function PafMathematicsChaptersPage() {
  return (
    <ProtectedRoute>
        ...
        <section className="container mx-auto px-4 py-12 sm:py-16 md:py-24">
            ...
            {chapters.map((chapter) => (
                ...
                <CardFooter>
                    ...
                    <Link href={`/paf/mathematics/${createSlug(chapter)}`}>
