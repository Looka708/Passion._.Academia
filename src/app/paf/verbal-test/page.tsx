...
import { chapters } from "@/lib/data/paf/verbal-test";

const createSlug = (title: string) => {
  return title.toLowerCase().replace(/ /g, '-').replace(/:/g, '').replace(/\(/g, '').replace(/\)/g, '');
};

export default function PafVerbalTestChaptersPage() {
  return (
    <ProtectedRoute>
        ...
        <section className="container mx-auto px-4 py-12 sm:py-16 md:py-24">
            ...
            {chapters.map((chapter) => (
                ...
                <CardFooter>
                    ...
                    <Link href={`/paf/verbal-test/${createSlug(chapter)}`}>
